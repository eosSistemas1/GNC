...
namespace PetroleraManager.Web.Tramites
{
    public partial class ObleasIngresar : PageBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (Request.QueryString["id"] != null)
                {
                    ...
                    this.btnVerImagenes.Visible = true;
                }
        ...
        {
            txtNroObleaAnterior.Visible = (cboTipoOperacion.SelectedValue != TIPOOPERACION.Conversion);
            btnBuscarOblea.Visible = false;

            if (cboTipoOperacion.SelectedValue == TIPOOPERACION.Conversion)
            {
            ...
            uscCargarReguladores1.TipoperacionID = cboTipoOperacion.SelectedValue;
            uscCargarCilindrosValvulas1.TipoperacionID = cboTipoOperacion.SelectedValue;
        }
        ...
        private Boolean BuscarFichaTecnicaPorID(Guid idOblea)
        {
            ...
            Obleas oblea = this.ObleasLogic.ReadDetalladoByID(idOblea);
        ...
        /// </summary>
        private void BindearFichaTecnica(Obleas oblea)
        {
            ...
            Boolean esRevision = cboTipoOperacion.SelectedIndex > -1 && (
                                 cboTipoOperacion.SelectedValue == TIPOOPERACION.RevisionAnual ||
                                 cboTipoOperacion.SelectedValue == TIPOOPERACION.RevisionCRPC ||
                                 cboTipoOperacion.SelectedValue == TIPOOPERACION.Modificacion);
            ...
            this.cboTipoOperacion.SelectedValue = oblea.Operaciones.ID;
        ...
        private String ValidarOblea(Boolean esModificacion)
        {
            ...
            if (cboTipoOperacion.SelectedValue == Guid.Empty) msjValida += "- Seleccione tipo de operación <br>";
            ...
            //else
            //{
            //    String msjValidacioCil = "El nro. de certificado PH de/los cilindro/s {0} no es correcto. </br>";
            //    List<String> seriesCil = new List<String>();
            //    foreach (var cilindro in uscCargarCilindrosValvulas1.CilindrosCargados)
            //    {
            //        int anio = int.Parse(cilindro.CilRevAnio) < 60 ? int.Parse(20 + cilindro.CilRevAnio) : int.Parse(19 + cilindro.CilRevAnio);
            //        DateTime fechaRev = new DateTime(anio, int.Parse(cilindro.CilRevMes), 1);
            //        DateTime fechaTope = new DateTime(2009, 10, 1);
            //        if ((fechaRev >= fechaTope)
            //            && (cilindro.CRPCCilID != CrossCutting.DatosDiscretos.CRPC.FAB)
            //            && (String.IsNullOrEmpty(cilindro.NroCertificadoPH) || cilindro.CRPCCilID == Guid.Empty))
            //        {
            //            if (cilindro.NroCertificadoPH.Length < 4 || cilindro.NroCertificadoPH.ToUpper().Substring(0, 4) != cilindro.CRPCCil.ToUpper().Trim())
            //                seriesCil.Add(cilindro.NroSerieCil);
            //        }
            //    }

            //    if (seriesCil != null && seriesCil.Any())
            //    {
            //        msjValida += String.Format(msjValidacioCil, String.Join(";", seriesCil));
            //    }
            //}
            ...
            if (cboTipoOperacion.SelectedValue != Guid.Empty && cboTipoOperacion.SelectedValue != CrossCutting.DatosDiscretos.TIPOOPERACION.Conversion &&
                String.IsNullOrWhiteSpace(txtNroObleaAnterior.Text)) msjValida += "- Debe ingresar numero de oblea anterior <br>";
        ...
        private void GrabarOblea(Guid idTallerRT)
        {
            using (TransactionScope ss = new TransactionScope())
            {
                try
                {
                    ...
                    String urlRetorno = "ObleasIngresar.aspx";
        ...
        private void GuardarFicha(Guid idOblea, VehiculosExtendedView vehiculo, Clientes cliente, Guid idTallerRT)
        {
            ...
            var taller = tallerLogic.Read(this.BuscarTaller1.SelectedValue.ID);
            ...
            oblea.IdUsuarioAlta = SiteMaster.IdUsuarioLogueado;
            oblea.FechaRealAlta = DateTime.Now;

            oblea.IdRTPEC = SiteMaster.RTPEC;
        ...
        private void GrabarCilindros(Guid idOblea)
        {
            ...
            foreach (ObleasCilindrosExtendedView gr in cilindros)
            {
                ...
                oc.ID = gr.ID;
                oc.IdOblea = idOblea;
                oc.IdCilindroUnidad = this.CilindrosUnidadLogic.LeerCrearCilindroUnidad(CodigoCIL, SerieCIL, FabMes.ToString(), FabAnio.ToString());
            ...
            foreach (ObleasValvulasExtendedView valvula in valvulas)
            {
                ...
                oV.ID = valvula.ID != Guid.Empty ? valvula.ID : Guid.NewGuid();
                oV.IdObleaCilindro = valvula.IdObleaCil;
                oV.IdValvulaUnidad = this.ValvulaUnidadLogic.LeerCrearValvulaUnidad(CodigoVAL, SerieVAL);
        ...
        /// </summary>        
        private void ImprimirOblea(Guid idOblea, String urlRetorno)
        {
            String urlObleasImprimir = SiteMaster.UrlBase + @"02.Tramites/ObleasImprimir.aspx?id=" + idOblea;
            PrintBoxCtrl1.PrintBox("Imprimir", urlObleasImprimir, String.Empty, urlRetorno);
        }
        #endregion
        ...
        protected void btnBuscarOblea_Click(object sender, EventArgs e)
        {
            ...
            if (!ObleasLogic.ExisteObleaConNroObleaAnterior(txtNroObleaAnterior.Text))
            {
            ...
            {
                String mensaje = String.Format("Ya existe una oblea con el nro de oblea anterior {0}.", txtNroObleaAnterior.Text);
        ...
        private Boolean BuscarFichaTecnica()
        {
            ...
            Obleas oblea = this.ObleasLogic.ReadDetalladoByObleaAnterior(txtNroObleaAnterior.Text);

            if (oblea != null)
            {
                Obleas o = this.PrepararObleaParaBindear(oblea);
        ...
        /// </summary>        
        private Obleas PrepararObleaParaBindear(Obleas oblea)
        {
            Obleas o = new Obleas();
            ...
            o.IdUso = oblea.IdUso;
            o.Operaciones = new Operaciones() { ID = cboTipoOperacion.SelectedValue, Descripcion = cboTipoOperacion.SelectedText };
            ...
            o.IdUsuarioAlta = SiteMaster.IdUsuarioLogueado;
        ...
        {
            String urlObleasImprimir = SiteMaster.UrlBase + @"02.Tramites/VerImagenes.aspx?id=" + this.ObleaID;
            PrintBoxCtrl1.PrintBox("Imprimir", urlObleasImprimir);
        }
